...
loss = tf.reduce_mean(tf.square(tf.sub(winModel.y_, winModel.y))) + tf.add_n([tf.nn.l2_loss(v) for v in train_vars]) * L2NormConst
...
for epoch in range(100):
    ...
    for i in range(1,5509):
        if i == 3286: # Turns out this game got postponed due to a leaky roof and now nobody has the data except for the final score
            continue
        with open('C:\\Users\\Douglas\\Documents\\Visual Studio 2012\\Projects\\NCAABasketball\\NCAABasketball\\2013-2014\game' + str(i) + '.txt', 'r') as csvfile:
            ...
            if team1[0] in teams and team2[0] in teams:
                ...
                y_data += [[1] if (float(team1[22]) - float(team2[22]) >= 0) else [0]]
                y_data += [[1] if (float(team2[22]) - float(team1[22]) >= 0) else [0]]
                if i == 5516 or len(x_data) == 50:
                    train_step.run(feed_dict={winModel.x: x_data, winModel.y_: y_data, winModel.keep_prob: 0.8})

                    loss_value = loss.eval(feed_dict={winModel.x: x_data, winModel.y_: y_data, winModel.keep_prob: 1.0})
